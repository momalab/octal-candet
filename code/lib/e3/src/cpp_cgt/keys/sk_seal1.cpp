    ...
    : PrivKey(name, seed, lam), ek(name)
{
    ...
    ///std::transform( encoder.begin(), encoder.end(), encoder.begin(), [](unsigned char const & c) { return ::tolower(c); } );
    ///std::transform( encoder.begin(), encoder.end(), encoder.begin(), std::tolower );
